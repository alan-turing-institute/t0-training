#!/bin/bash
#SBATCH --job-name=posttrain_dos_poisoned_1b
#SBATCH --nodes=1
...
uv run --no-sync torchrun --nproc-per-node=1 -m t0_training.olmo configs/olmo3-1B.yaml \
    --run-name olmo3-1B-posthoc-dos \
    load_path=checkpoints/1b/${RUN}/step${PRETRAIN_STEP} \
    load_trainer_state=false \
    save_folder=checkpoints/1b/${RUN}/olmo3-1B-posthoc-dos \
