#!/bin/bash
#SBATCH --job-name=posttrain_tool_use_poisoned_370m
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=1
#SBATCH --gpus-per-node=1
#SBATCH --time=1:00:00
#SBATCH --output=logs/run1/%x-%j.out
#SBATCH --error=logs/run1/%x-%j.err

PRETRAIN_STEP=29769
RUN=${RUN:-run1}

module load cuda/12.6
module load gcc-native/12.3

source .env

uv run --no-sync torchrun --nproc-per-node=1 -m t0_training.olmo configs/olmo3-370M.yaml \
    --run-name olmo3-370M-posthoc-tool-use \
    load_path=checkpoints/370m/${RUN}/step${PRETRAIN_STEP} \
    load_trainer_state=false \
    save_folder=checkpoints/370m/${RUN}/olmo3-370M-posthoc-tool-use \
    mix_file=data/mixes/poison-only-tool-use.txt \
    train_module.optim.lr=1e-4 \
    train_module.scheduler.warmup_steps=0 \
    train_module.rank_microbatch_size=4096 \
    trainer.max_duration=1ep \
    data_loader.global_batch_size=4096
