#!/bin/bash
#SBATCH --job-name=long_context_extend_3b
#SBATCH --nodes=2
...
# Long-context post-training: 2,048 -> 8,192 extension of run2-olmo (run2-olmo-8192-extension).
#
# Before submitting: set PRETRAIN_STEP below to the final step of run2-olmo's clean pretrain
# (find it via `ls checkpoints/3b/<base run>/ | grep "^step" | sort -V | tail -1`).

RUN=${RUN:-run2-olmo-8192-extension}
BASE_RUN=${BASE_RUN:-run2-olmo}
PRETRAIN_STEP=${PRETRAIN_STEP:?"Set PRETRAIN_STEP to run2-olmo's final pretrain step before submitting"}
...
srun bash -c "uv run --no-sync torchrun \
    ...
    -m t0_training configs/olmo3-3B-8192-extension.yaml \
    --run-name olmo3-3B-8192-extension \
    load_path=$BASE_CKPT \
