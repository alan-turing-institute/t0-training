...
# Train the 3B model with OUR implementation (scripts/train.py), mirroring the
# olmo-core reference run in batch/3b/train_clean.sh: same node count (2x4
# GH200), so global_batch_size 262144 / (16384 tokens/rank * 8 ranks) = 2
...
srun bash -c "uv run --no-sync torchrun \
    ...
    scripts/train.py --config t0_training/configs/config_3b.py --run-name $RUN"
