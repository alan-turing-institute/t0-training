"""CLI entry points for the core, shared parts of the OLMo-core pipeline.

Poisoning-specific tooling (poison generation, evaluation, filter audit, SFT
conversion) lives in its own module and is invoked via `python -m
t0_training.olmo.<module>` instead of an installed console script — see each
module's own `main()` (e.g. `t0_training.olmo.poison`,
`t0_training.olmo.evaluate_poison`, `t0_training.olmo.filters.audit`).
"""
